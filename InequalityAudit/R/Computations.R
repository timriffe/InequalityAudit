...
(Test <- w_iteration(w=c(1,1,1)/3,DHS, k = 1/3, CCvar = "CC_Poor"))
...
# stats for Table 5
w_statsExample <- function(w, DHS, k, CCvar = "CC_Poor"){
...
# step 8:
	...
	P_AFi        		<- as.matrix(Sumsii[,as.character(k),with=FALSE]) / denom
	P_CCi        		<- as.matrix(Sumsiii[,"CCcen",with=FALSE]) / denom
    POut <- data.frame(P_AF = P_AFi, P_CC = P_CCi)
	rownames(POut) <- Sumsii$country
	POut
}
# for table 5
CCPoor <- w_statsExample(w=c(1,1,1)/3, DHS, k = 1/3, CCvar = "CC_Poor")
CCPoor2 <- w_statsExample(w=c(1,1,1)/3, DHS, k = 1/3, CCvar = "CC_Poor2")

Tab5 <- cbind(CCPoor, CCPoor2)
colnames(Tab5) <- c("P_AF_Poor","P_CC_Poor","P_AF_Poor2","P_CC_Poor2")
write.table(Tab5,file="Table5.csv",sep=",")